...
def test_equivalence_with_positional_model():
    ...
    for i in range(10):
        ...
        old_output = old_model.run_forward(input_sample)
        new_output = new_model.run_forward(input_sample)
...
def test_intervention_on_positional_entity():
    ...
    # Normal run
    normal = model.run_forward(input_sample)
    print(
        f"Normal: Query Ann (g1_e0) -> position {normal['positional_entity_g1_e0']} -> answer {normal['raw_output']}"
    )
    ...
    intervened = model.run_interchange(
        input_sample,
        {
            "positional_entity_g1_e0": counterfactual_sample
        },  # Make Ann appear at position 2
    )
...
def test_intervention_on_positional_query():
    ...
    normal = model.run_forward(input_sample)
    print(
        f"Normal: positional_query_e0={(normal['positional_query_e0'])}, answer={normal['raw_output']}"
    )
    ...
    intervened = model.run_interchange(
        input_sample,
        {
            "positional_query_e0": counterfactual_sample
        },  # Force search to return group 0
    )
...
def test_intervention_on_positional_answer():
    ...
    normal = model.run_forward(input_sample)
    print(
        f"Normal: positional_answer={normal['positional_answer']}, answer={normal['raw_output']}"
    )
    ...
    intervened = model.run_interchange(
        input_sample,
        {"positional_answer": counterfactual_sample},  # Force retrieval from group 2
    )
