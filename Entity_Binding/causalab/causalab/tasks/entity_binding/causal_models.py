...
def create_direct_causal_model(config: EntityBindingTaskConfig) -> CausalModel:
    ...
    def generate_raw_input(*args):
        ...
        question_template = remaining_args[1]
        query_group = remaining_args[2]
        query_indices = remaining_args[3]
    ...
    def generate_raw_output(*args):
        ...
        if query_group < active_groups and answer_index < entities_per_group:
            entity_idx = query_group * config.max_entities_per_group + answer_index
...
def create_positional_causal_model(config: EntityBindingTaskConfig) -> CausalModel:
    ...
    def extract_query_entity(*args):
        ...
        query_group = remaining_args[0]
        query_indices = remaining_args[1]

        # Extract all query entities as a tuple
        query_entities = []
        for query_entity_idx in query_indices:
            entity_idx = query_group * config.max_entities_per_group + query_entity_idx
            if entity_idx < len(entity_values):
                query_entities.append(entity_values[entity_idx])
            else:
                query_entities.append(None)
    ...
    def search_for_query_entity(*args):
        ...
        query_entity = remaining_args[0]  # This is now a tuple of entities
        query_indices = remaining_args[1]
        active_groups = remaining_args[2]
        ...
        for g in range(active_groups):
            ...
            for i, query_entity_idx in enumerate(query_indices):
                entity_idx = g * config.max_entities_per_group + query_entity_idx
                if entity_idx >= len(entity_values):
                    all_match = False
                    break
                if (
                    i < len(query_entity)
                    and entity_values[entity_idx] != query_entity[i]
                ):
                    all_match = False
                    break
    ...
    def generate_raw_output(*args):
        ...
        positional_query_group = remaining_args[0]  # Use POSITIONAL group (from search)
        answer_index = remaining_args[1]
        active_groups = remaining_args[2]
        entities_per_group = remaining_args[3]
        ...
        if (
            positional_query_group is not None
            and positional_query_group < active_groups
            and answer_index < entities_per_group
        ):
            entity_idx = (
                positional_query_group * config.max_entities_per_group + answer_index
            )
            if entity_idx < len(entity_values):
                answer_entity = entity_values[entity_idx]
                if answer_entity is not None:
...
def create_positional_entity_causal_model(
    ...
    def create_positional_query_mechanism(entity_position):
        ...
        def mechanism(*args):
            ...
            remaining = args[2 * num_entities :]

            query_group = remaining[0]
            query_indices = remaining[1]
            active_groups = remaining[2]
            ...
            entity_idx = query_group * config.max_entities_per_group + entity_position
            if entity_idx >= len(entity_values):
                return ()

            query_entity = entity_values[entity_idx]
