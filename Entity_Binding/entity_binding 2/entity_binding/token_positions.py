...
def _build_values_dict(
    ...
    active_groups = input_sample.get("active_groups", config.max_groups)
    entities_per_group = input_sample.get("entities_per_group", config.max_entities_per_group)
    query_indices = input_sample.get("query_indices", (0,))
    ...
    for e in range(entities_per_group):
        ...
        values[role_name] = input_sample.get(f"query_e{e}", "")
...
def get_entity_token_positions(
    ...
    active_groups = input_sample.get("active_groups", config.max_groups)
    query_indices = input_sample.get("query_indices", (0,))
    answer_index = input_sample.get("answer_index", 1)

    mega_template_str = config.build_mega_template(active_groups, query_indices, answer_index)
    ...
    if token_idx is not None:
        if not positions:
            raise ValueError(f"No tokens found for entity at group {group_idx}, position {entity_idx}")
