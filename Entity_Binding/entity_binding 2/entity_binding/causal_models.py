...
from neural.token_position_builder import Template
...
def sample_valid_entity_binding_input(
    ...
    for attempt in range(max_attempts):
        ...
        # Query entity variables (query_e0, query_e1, ...) will be set after entities are sampled
        ...
        if all_valid:
            ...
            for e in range(config.max_entities_per_group):
                input_sample[f"query_e{e}"] = input_sample.get(f"entity_g{query_group}_e{e}")
            return input_sample

    # If we failed after max_attempts, return a simple valid sample
    query_group = 0  # Fallback query group
    input_sample = {
        "query_group": query_group,
        "query_indices": (0,),
        "answer_index": 1,
    ...
    # Set query entity variables from the query group's entities
    for e in range(config.max_entities_per_group):
        input_sample[f"query_e{e}"] = input_sample.get(f"entity_g{query_group}_e{e}")
...
def create_positional_entity_causal_model(
    ...
    # Query entity variables - one for each possible position in the query group
    for e in range(config.max_entities_per_group):
        variables.append(f"query_e{e}")

    # Control variables
    variables.extend(
        [
            "query_group",  # Which group contains the query entity
    ...
    # Query entity values - same as entity pools at each position (computed from query_group + entities)
    for e in range(config.max_entities_per_group):
        key = f"query_e{e}"
        if e in config.entity_pools:
            values[key] = config.entity_pools[e] + [None]
    ...
    # Query entity variables depend on the statement entities (they are copies from the query group)
    # These are input variables, set externally based on which group is being queried
    for e in range(config.max_entities_per_group):
        parents[f"query_e{e}"] = []
    ...
    # Query entity vars for positional query dependencies
    ...
    parents["raw_input"] = entity_vars + query_entity_vars + [
        "statement_template",
        "question_template",
        "query_indices",
        "answer_index",
    ...
    for g in range(config.max_groups):
        for e in range(config.max_entities_per_group):
            ...
            if e in config.entity_pools:
                ...
                mechanisms[key] = lambda pool=pool: random.choice(pool)
            else:
                mechanisms[key] = lambda: None
    ...
    for g in range(config.max_groups):
        for e in range(config.max_entities_per_group):
            key = f"positional_entity_g{g}_e{e}"
            mechanisms[key] = (
                lambda entity_val, g=g: g if entity_val is not None else None
            )

    # Query entity mechanisms - sample from entity pools (set externally in practice)
    for e in range(config.max_entities_per_group):
        key = f"query_e{e}"
        if e in config.entity_pools:
            pool = config.entity_pools[e]
    ...
    mechanisms.update(
        {
            "query_group": lambda: random.randint(0, config.max_groups - 1),
            "query_indices": lambda: tuple(
                [random.randint(0, config.max_entities_per_group - 1)]
            ),
            "answer_index": lambda: random.randint(
                0, config.max_entities_per_group - 1
            ),
            "active_groups": lambda: random.randint(2, config.max_groups),
            "entities_per_group": lambda: config.max_entities_per_group,
            "statement_template": lambda: config.statement_template,
        }
    )

    # Question template selection mechanism
    def select_question_template(query_indices, answer_index):
    ...
    def create_positional_query_mechanism(entity_position):
        ...
        def mechanism(*args):
            # Parse arguments
            # Args order: entity_vars, positional_entity_vars, query_entity_vars, query_indices, active_groups, entities_per_group
            num_entities = config.max_groups * config.max_entities_per_group
            entity_values = args[:num_entities]
            positional_entity_values = args[num_entities : 2 * num_entities]
            num_query_entities = config.max_entities_per_group
            query_entity_values = args[2 * num_entities : 2 * num_entities + num_query_entities]
            remaining = args[2 * num_entities + num_query_entities :]

            query_indices = remaining[0]
            active_groups = remaining[1]
            ...
            # Get the query entity at this position from query_e{entity_position}
            query_entity = query_entity_values[entity_position]
            ...
            for g in range(active_groups):
                idx = g * config.max_entities_per_group + entity_position
                if idx < len(entity_values):
                    if entity_values[idx] == query_entity:
                        # Found matching entity at this position
                        pos_idx = g * config.max_entities_per_group + entity_position
                        if pos_idx < len(positional_entity_values):
                            group_pos = positional_entity_values[pos_idx]
    ...
    def compute_positional_answer(*args):
        """Compute the single position by intersecting all positional queries."""
        positional_queries = args[: config.max_entities_per_group]
        query_indices = args[config.max_entities_per_group]
        ...
        for i, entity_idx in enumerate(query_indices):
            if entity_idx < len(positional_queries):
                query_positions = positional_queries[entity_idx]
                if query_positions:
    ...
    def generate_raw_input(*args):
        ...
        # Args order: entity_vars, query_entity_vars, statement_template, question_template,
        #             query_indices, answer_index, active_groups, entities_per_group
        num_entities = config.max_groups * config.max_entities_per_group
        entity_values = args[:num_entities]
        num_query_entities = config.max_entities_per_group
        query_entity_values = args[num_entities : num_entities + num_query_entities]
        remaining_args = args[num_entities + num_query_entities :]

        # remaining_args order: statement_template, question_template,
        #                       query_indices, answer_index, active_groups, entities_per_group
        query_indices = remaining_args[2]
        # Convert list to tuple (HuggingFace datasets convert tuples to lists)
        if isinstance(query_indices, list):
            query_indices = tuple(query_indices)
        answer_index = remaining_args[3]
        active_groups = remaining_args[4]
        entities_per_group = remaining_args[5]

        # Build entity dictionary
        entity_dict = {}
        for g in range(config.max_groups):
            for e in range(config.max_entities_per_group):
                idx = g * config.max_entities_per_group + e
                entity_dict[f"entity_g{g}_e{e}"] = entity_values[idx]
        ...
        try:
            ...
            # Question entities: use query_e0, query_e1, etc. directly
            values["query_entity"] = query_entity_values[query_indices[0]]
            for e in range(entities_per_group):
                role_name = config.entity_roles.get(e, f"entity{e}")
                values[role_name] = query_entity_values[e]

            # mega_template already includes prompt_prefix and prompt_suffix
    ...
    def generate_raw_output(*args):
        ...
        entity_values = args[: config.max_groups * config.max_entities_per_group]
        remaining_args = args[config.max_groups * config.max_entities_per_group :]

        positional_answer = remaining_args[0]
        answer_index = remaining_args[1]
        active_groups = remaining_args[2]
        entities_per_group = remaining_args[3]
        ...
        if (
            ...
            entity_idx = (
                positional_answer * config.max_entities_per_group + answer_index
            )
            if entity_idx < len(entity_values):
                answer_entity = entity_values[entity_idx]
